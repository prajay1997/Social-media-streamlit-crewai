...
def initialize_tools_and_keys():
    ...
    serper_api_key_val = None

    # Attempt to load from Streamlit secrets
    try:
        if hasattr(st, 'secrets'): # Check if st.secrets attribute exists
            openai_api_key_val = st.secrets.get("OPENAI_API_KEY")
            serper_api_key_val = st.secrets.get("SERPER_API_KEY")
    except st.errors.StreamlitAPIException: 
        # Catches StreamlitSecretNotFoundError if secrets.toml is missing or other st.secrets issues.
        # Silently pass, as we want to fall back to environment variables.
        # st.sidebar.info("API keys not found in st.secrets (e.g., missing secrets.toml). Trying environment variables.") # Optional debug
        pass 
    except Exception: 
        # Catch any other unexpected error, though StreamlitAPIException should cover most st.secrets issues.
        # st.sidebar.error("Unexpected error checking st.secrets for API keys. Trying environment variables.") # Optional debug
        pass

    # Fallback to environment variables if not found in Streamlit secrets
    ...
    if openai_api_key_val:
        ...
        # This warning will be shown if GPT is selected later and key is still missing
        pass
    ...
    if serper_api_key_val:
        ...
        # Warning will be shown in the UI if Serper is needed and key is still missing
        pass 

    # Instantiate Search Tool
    search_tool_instance = None # Reset
    # Check os.getenv("SERPER_API_KEY") directly as it's the effective value after trying secrets and .env
...
def create_llm_streamlit(use_gpt=True):
    """Creates and returns an LLM instance based on user selection."""
    if use_gpt:
        if ChatOpenAI is None:
            st.error("Langchain OpenAI component not loaded. Cannot use GPT.")
            return None
        if not os.getenv("OPENAI_API_KEY"): # Checks effective environment variable
...
def run_main_app():
    ...
    if submit_button:
        if not political_entity_name_input:
            ...
            if search_tool_instance is None or (isinstance(search_tool_instance, BaseTool) and "Dummy" in search_tool_instance.name):
                 # Check if SERPER_API_KEY is effectively missing after trying secrets and .env
                 if not os.getenv("SERPER_API_KEY"): # This check is important
...
# --- App Entry Point ---
# Directly run the main application logic without password check
